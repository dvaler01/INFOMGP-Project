...
bool key_down(igl::opengl::glfw::Viewer &viewer, unsigned char key, int modifier)
{
  ...
  if (key == 'P')
  {
	  if (!viewer.core.is_animating) {
		  ...
		  scene.updateScene(timeStep, CRCoeff, tolerance, maxIterations, V, true, friction);
		  currTime += timeStep;
		  update_mesh(viewer);
  ...
  if (key == 'S')
  {
    if (!viewer.core.is_animating){
      ...
      scene.updateScene(timeStep, CRCoeff, tolerance, maxIterations,V,false, friction);
      currTime+=timeStep;
      update_mesh(viewer);
      std::cout <<"currTime: "<<currTime<<std::endl;
      return true;
...
class CustomMenu : public igl::opengl::glfw::imgui::ImGuiMenu
{
  ...
  virtual void draw_viewer_menu() override
  {
    ...
    if (ImGui::CollapsingHeader("Algorithm Options", ImGuiTreeNodeFlags_DefaultOpen))
    {
	  ...
	  ImGui::InputFloat("friction", &friction, 0, 0, 3);
